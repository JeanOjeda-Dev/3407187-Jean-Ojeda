...
const TAX_RATE = 0.19; // definimos la tasa del IVA del 19% como una constante global para cálculos impositivos
const CURRENCY = "COP"; // establecemos el código de la moneda local para formatear los resultados económicos

// cree un array de objetos con montos usando separadores numéricos (_) para mayor legibilidad en el código
const facturasPyme = [
...
// implemente una arrow function que recibe un objeto factura y devuelve un string formateado con puntos de miles
// eliminé los emojis para mantener un estilo de reporte estrictamente profesional
...
// esta es una función pura que calcula el valor del IVA a partir de un monto base y una tasa opcional
const calcularIVA = (montoBase, tasa = TAX_RATE) => {
    // multiplicamos el monto por la tasa para obtener el valor del impuesto
...
// función que verifica si una factura es válida para el cierre contable basándose en su propiedad 'activo'
...
// función para formatear el resumen de totales con etiquetas personalizables y formato de moneda local
const formatResumen = (valor, etiqueta = "TOTAL", moneda = CURRENCY) => {
    // aca retorne el string combinado usando toLocaleString para mostrar los puntos separadores de miles
...
// inicie la generación del reporte visual en la consola utilizando los decoradores de línea
...
// verifique si el array tiene datos para procesar antes de iniciar los bucles
if (facturasPyme.length === 0) {
    ...
    let contadorLineas = 1; // inicie un contador para numerar las líneas del reporte
    
    for (const item of facturasPyme) {
        // aca esto recorre las facturas y use nuestra arrow function 'formatFactura' como callback de formato
        console.log(`  ${contadorLineas}. ${formatFactura(item)}`);
        // aca incremente el contador en cada iteración
        contadorLineas++;
    }
    ...
    for (const factura of facturasPyme) {
        // aplicamos la función de validación 'esFacturaValida' para filtrar registros inactivos
        if (esFacturaValida(factura)) {
            // si es válida, sumamos su monto al acumulador general
            sumaSubtotal += factura.monto;
            // aumentamos el conteo de registros efectivos
            facturasProcesadas++;
        }
    }
